...
module Memory = struct
  ...
  let layout =
    let low = range 2 (limit / 2) in (* Avoid the first address, = ISR *)
    let high = range (limit / 2) (limit - 2) in (* Avoid the last address *)
    oneof [pair low high; pair high low] >|= fun (data, code) ->
    { data; code; isr = 0 } (* WARNING: ISR is set to always zero in these examples! *)
end
...
module Register = struct
  ...
  (* Any register *)
  let any_register = oneofl [PC; SP; SR; R3; R4; R5; R6; R7; R8; R9; R10; R11; R12; R13; R14; R15]
  ...
  let gp_register = oneofl [R3; R4; R5; R6; R7; R8; R9; R10; R11; R12; R13; R14; R15]
end
...
module Configuration = struct
  ...
  let default_memory = memory_init () (* Careful! Global because of efficiency, should not be overwritten by tests *)

  let default_io_device = default_io_device (* Careful! Global because of efficiency, should not be overwritten by tests *)

  let configuration_minimal = Memory.layout >|= fun layout -> init_configuration true layout default_io_device default_memory ()
end
