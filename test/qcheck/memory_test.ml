...
let test_byte_set_is_involutive =
    ...
    QCheck2.Test.make ~name:"memory set memory get of bytes returns the initial value" ~count:50 gen property
...
let test_word_set_is_involutive =
    ...
    in
    let gen = QCheck2.Gen.triple memory address byte in
    QCheck2.Test.make ~name:"memory set memory get of words returns the initial value" ~count:50 gen property
