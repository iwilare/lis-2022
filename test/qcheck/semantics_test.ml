...
let test_operation name operation instruction =
  let property (c, r1, r2) =
    ...
    valid ==> (good && unchanged_r1 && (overflow || register_get c.r r2 = result))
  in
  let gen =
  ...
  in
  QCheck2.Test.make
    ~name:(name ^ " changes the first register with the correct value") ~count:500
...
let test_mov =
  let property (c, r1, r2) =
    ...
    valid ==> (good && before_r1 == register_get c.r r1
  ...
  in
  QCheck2.Test.make
    ~name:"MOV changes the second register with the correct value from the first" ~count:500 gen
    property
...
let test_j0_um_yes =
  let property (c, r, unprotected_addr) =
    ...
    valid ==> (good && unchanged_r && QCheck2.(not z ==> Word.(c.r.pc = before_pc + Word.from_int (size i))))
  in
  let gen =
      (Configuration.configuration_unprotected_minimal ()) >>= fun config ->
...
let test_j0_um_no =
  let property (c, r, unprotected_addr) =
    ...
    valid ==> (good && unchanged_r && QCheck2.(z ==> (c.r.pc = register_get c.r r)))
  in
  let gen =
      (Configuration.configuration_unprotected_minimal ()) >>= fun config ->
      QCheck2.Gen.triple (pure config) Register.gp_register (Memory.unprotected_address config.layout)
...
let test_jmp_um =
  ...
  in
  QCheck2.Test.make
    ~name:"JMP (UM) sets the PC with correct value from the register"
    ~count:500 gen property
...
let test_in_device =
  let property (c, r) =
    ...
    | Some (w, d') ->
        let valid = mac_valid c i in
        let good = step c i |> is_ok in
        let (correct_state, _, _) = advance c.io_device (cycles i - 1) (d', 0, None) in
        valid ==> (good &&
  ...
  let gen =
    ...
    pair (Configuration.configuration_unprotected_minimal () ~io_device:io_device) Register.gp_register
  in
  QCheck2.Test.make ~name:"IN performs a read from the device" ~count:500 gen property
...
let test_out_device =
  ...
  let gen =
    ...
    triple (Configuration.configuration_unprotected_minimal () ~io_device:io_device) Register.gp_register (0 -- write_transitions_max >|= Word.from_int)
  in
  QCheck2.Test.make ~name:"OUT performs a read from the device" ~count:500 gen property
