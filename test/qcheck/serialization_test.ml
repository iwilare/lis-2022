...
let test_enc_dec_of_registers =
  ...
  QCheck2.Test.make ~name:"Register decode the inverse of encode" ~count:20000 gen
    property

let test_enc_dec_of_instructions =
  let property i = encode i |> decode = Some i in
  let gen = Instructions.instr in
  QCheck2.Test.make ~name:"Instruction decode is the inverse of encode" ~count:20000
  ~print:Lis2022.Instr.string_of_instr
    gen property
...
let test_enc_dec_of_instructions_in_memory =
  ...
  QCheck2.Test.make ~name:"Instruction decode in memory is the inverse of encode in memory" ~count:20000
