  ...
  (let@ arrival_time in
   ...
   | Some ta when flag_gie -> (
       ...
       | Some UM ->
           ...
           >> (* Jump to the ISR *)
           rset PC isr >> rset SR Word.zero
           >> rset SP Word.(sp - from_int 4)
           >> reset_last_arrival_time
           >> (* Clear last arrival time, but accept the next new interrupts *)
           advance_config 6
       ...
       | Some PM ->
           ...
           let@ new_backup =
             generate_backup (if post_padding then t_pad else 0)
           in
           let@ isr in
           set_backup @@ Some new_backup
           >> clear_registers >> rset PC isr
           >> advance_config (6 + if pre_padding then k else 0)
           >> reset_last_arrival_time
       (* Ignore any interrupt arrived during this last advance *))
   | _ -> ok)
    c
...
module Sancus_low = struct
  let interrupt_logic c =
    sancus_interrupt_logic ~pre_padding:true ~post_padding:true c
end

module Sancus_pre_pad = struct
  let interrupt_logic c =
    sancus_interrupt_logic ~pre_padding:true ~post_padding:false c
end

module Sancus_no_pad = struct
  let interrupt_logic c =
    sancus_interrupt_logic ~pre_padding:false ~post_padding:false c
end
