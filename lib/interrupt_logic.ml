...
let interrupt_logic ~activate_padding () =
  let* c = get in
...
module Sancus_unsafe = struct
  let interrupt_logic () = interrupt_logic ~activate_padding:false () 
end

module Sancus_low = struct
  let interrupt_logic () = interrupt_logic ~activate_padding:true () 
end

module Sancus_high = struct
  let interrupt_logic () = ok
end
