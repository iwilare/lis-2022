...
let sr c = c.r.sr
let sp c = c.r.sp
let load_here c a = memory_get a c.m
let with_memory f c = f c.m

let string_of_config c =
  "Instruction: "
  ^ Option.fold
      (fetch_and_decode c.r.pc c.m)
      ~none:"<invalid>" ~some:string_of_instr
  ^ "\n" ^ "Layout: {" ^ string_of_layout c.layout ^ "}" ^ " CPU MODE: "
  ^ Option.fold (cpu_mode c) ~none:"<invalid>" ~some:string_of_cpu_mode
  ^ "\nClock: "
  ...
  ^ Option.fold ~none:"-"
      ...
      c.b
  ^ " [IO: "
  ^ string_of_io_possibilities
      (io_device_get_possibilities c.io_device c.io_state)
