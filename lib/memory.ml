...
let memory_get_byte (a : address) (m : memory) =
  ...
  else failwith "Invalid memory access"

let memory_set_byte (a : address) (b : byte) (m : memory) =
  m |> List.remove_assoc a |> List.cons (a, b)

let memory_get (a : address) (m : memory) =
  Word.(compose_bytes (memory_get_byte (Word.inc a) m)) (memory_get_byte a m)

let memory_set (a : address) (w : word) (m : memory) =
  Word.decompose_bytes w |> fun (h, l) ->
  m |> memory_set_byte a l |> memory_set_byte (Word.inc a) h
...
let string_of_memory m =
  m
  |> List.sort (fun (a, _) (b, _) -> Word.to_int a - Word.to_int b)
...
let is_in_range range addr = range.range_start <= addr && addr < range.range_end
