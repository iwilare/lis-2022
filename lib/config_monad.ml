...
type ('a, 'io_device) m =
  'io_device config -> [ `ok of 'a | `halt of halt_error ] * 'io_device config

let (( >>= ) :
      ('a, 'io_device) m -> ('a -> ('b, 'io_device) m) -> ('b, 'io_device) m) =
 fun ma mf c ->
  match ma c with `halt e, _ -> (`halt e, c) | `ok a, c' -> mf a c'

let (pure : 'a -> ('a, 'io_device) m) = fun v c -> (`ok v, c)
let ( let* ) = ( >>= )
let ( >> ) a b = a >>= fun () -> b
...
let (gets : ('io_device config -> 'a) -> ('a, 'io_device) m) =
 fun f c -> (`ok (f c), c)

let (set : 'io_device config -> (unit, 'io_device) m) = fun c _ -> (`ok (), c)

let (modify : ('io_device config -> 'io_device config) -> (unit, 'io_device) m)
    =
 fun f c -> (`ok (), f c)

let (ok : (unit, 'io_device) m) = fun c -> (`ok (), c)
let (halt : halt_error -> ('a, 'io_device) m) = fun err c -> (`halt err, c)
let ( let@ ) a f = gets a >>= f

let catch_halt (m : (unit, 'io_device) m) : (halt_error option, 'io_device) m =
 fun c ->
  match m c with `halt e, _ -> (`ok (Some e), c) | `ok (), c' -> (`ok None, c')
...
let rmodify r f =
  modify (fun c ->
      { c with r = register_set c.layout r (f (register_get r c.r)) c.r })
...
let increment_current_clock c =
  modify (fun c -> { c with current_clock = c.current_clock + 1 }) c

let clear_registers c = modify (fun c -> { c with r = register_file_0 }) c

let reset_last_arrival_time c =
  modify (fun c -> { c with arrival_time = None }) c
...
let advance_config k =
  modify (fun c ->
      ...
      in
      { c with io_state; current_clock; arrival_time })
